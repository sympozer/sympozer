...
})(this, function ($)
{
    var Tokenfield = function (element, options)
    {
        ...
        this._triggerKeys =
            $.map(this._delimiters, function (delimiter)
            {
                return delimiter.charCodeAt(0)
            });
        var elRules = typeof window.getMatchedCSSRules === "function" ? window.getMatchedCSSRules(element) : null, elStyleWidth = element.style.width, elCSSWidth, elWidth = this.$element.width();
        if (elRules)
        {
        ...
        this.$input = $('<input type="text" class="token-input" autocomplete="off" />').appendTo(this.$wrapper).prop("placeholder", this.$element.prop("placeholder")).prop("id", id + "-tokenfield");
        ...
        if (!$.isEmptyObject(this.options.autocomplete))
        {
            var autocompleteOptions = $.extend({},
                this.options.autocomplete, {minLength: this.options.showAutocompleteOnFocus ? 0 : null, position: {my: this.direction + " top", at: this.direction + " bottom", of: this.$wrapper}});
    ...
    Tokenfield.prototype = {constructor: Tokenfield, createToken: function (attrs, triggerChange)
    {
        ...
        var _self = this, value = $.trim(attrs.value), label = attrs.label.length ? $.trim(attrs.label) : value;
        if (!value.length || (!label.length || value.length < this.options.minLength))
        {
            return;
        }
        var beforeCreateEvent = $.Event("beforeCreateToken");
        beforeCreateEvent.token = {value: value, label: label};
        ...
        if (!this.options.allowDuplicates &&
            $.grep(this.getTokens(), function (token)
            {
                return token.value === value
            }).length)
        {
            var duplicateEvent = $.Event("preventDuplicateToken");
            duplicateEvent.token = {value: value, label: label};
        ...
        {
            this.maxTokenWidth = this.$wrapper.width() - closeButton.outerWidth() - parseInt(closeButton.css("margin-left"), 10) - parseInt(closeButton.css("margin-right"), 10) - parseInt(token.css("border-left-width"), 10) - parseInt(token.css("border-right-width"), 10) - parseInt(token.css("padding-left"),
                10) - parseInt(token.css("padding-right"), 10);
        ...
        tokenLabel.text(label).css("max-width", this.maxTokenWidth);
    ...
    }, getTokenData                    : function (token)
    {
        ...
        {
            var $token = $(this);
            return{value: $token.attr("data-value") || $token.find(".token-label").text(), label: $token.find(".token-label").text()}
        }).get();
        if (data.length == 1)
        {
            data = data[0];
    ...
    {
        var self = this, tokens = [], activeClass = active ? ".active" : "";
    ...
    }, listen                          : function ()
    {
        ...
        this.$input.on("focus",
            $.proxy(this.focus, this)).on("blur", $.proxy(this.blur, this)).on("paste", $.proxy(this.paste, this)).on("keydown", $.proxy(this.keydown, this)).on("keypress", $.proxy(this.keypress, this)).on("keyup", $.proxy(this.keyup, this));
        ...
        this.$input.on("autocompletecreate",
            ...
            {
                var valueKey = "value";
                $.each(_self.$input.data("ttView").datasets,
                    function (i, set)
                    {
                        if (set.name === dataset)
                        {
        ...
        switch (e.keyCode)
        {
            ...
            case 39:
                if (this.$input.is(document.activeElement))
                {
                    ...
                    var next = this.$input.hasClass("tt-query") ? this.$input.parent().nextAll(".token:first") :
                               this.$input.nextAll(".token:first");
                    if (!next.length)
                    {
                        break;
                    }
            ...
            case 13:
                ...
                if (this.$input.hasClass("tt-query") && this.$wrapper.find(".tt-is-under-cursor").length)
                {
                    break;
                }
                if (this.$input.hasClass("tt-query") && this.$wrapper.find(".tt-hint").val().length)
                {
                    break;
                }
    ...
    }, blur                            : function (e)
    {
        ...
        if (!this.preventCreateTokens &&
            (this.$input.data("edit") && !this.$input.is(document.activeElement) || this.options.createTokensOnBlur))
        {
            this.createTokensFromInput(e);
        }
    ...
    }, next                            : function (add)
    {
        if (add)
        {
            var firstActive = this.$wrapper.find(".active:first"), deactivate = firstActive && this.firstActiveToken ? firstActive.index() < this.firstActiveToken.index() : false;
            if (deactivate)
            {
                return this.deactivate(firstActive)
            }
        }
        var active =
                this.$wrapper.find(".active:last"), next = active.nextAll(".token:first");
        if (!next.length)
        {
            this.$input.focus();
            return
    ...
    }, prev                            : function (add)
    {
        if (add)
        {
            var lastActive = this.$wrapper.find(".active:last"), deactivate = lastActive && this.firstActiveToken ? lastActive.index() > this.firstActiveToken.index() : false;
            if (deactivate)
            {
                return this.deactivate(lastActive)
            }
        }
        var active = this.$wrapper.find(".active:first"), prev = active.prevAll(".token:first");
        if (!prev.length)
        {
    ...
    }, activate                        : function (token, add, multi, remember)
    {
        ...
        if (this.$wrapper.find(".token.active").length === this.$wrapper.find(".token").length)
        {
            return;
        }
    ...
    }, deactivate                      : function (token)
    {
        ...
        }, edit                        : function (token)
        {
            ...
            var value = token.data("value"), label = token.find(".token-label").text();
            var beforeEditEvent = $.Event("beforeEditToken");
            beforeEditEvent.token = {value: value, label: label};
            beforeEditEvent.relatedTarget = token.get(0);
            this.$element.trigger(beforeEditEvent);
            if (!beforeEditEvent.token)
            {
                return;
            }
            ...
            this.$input.val(value).select().data("edit", true).width(tokenWidth)
    ...
    $.fn.tokenfield.defaults = {minWidth: 60, minLength: 0, allowDuplicates: false, autocomplete: {}, typeahead: {}, showAutocompleteOnFocus: false, createTokensOnBlur: false, delimiter: ",", beautify: true};
